...
class JobFixtures extends Fixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager) : void
    {
        for ($i = 100; $i <= 130; $i++) {
            ...
            $jobSensioLabs->setToken('job_sensio_labs' . $i);
        ...
        $expiredJob = new Job();
        $expiredJob->setCategory($manager->merge($this->getReference('category-programming')));
        $expiredJob->setType('full-time');
        $expiredJob->setCompany('Sensio Labs');
        $expiredJob->setLogo('sensio-labs.gif');
        $expiredJob->setUrl('http://www.sensiolabs.com/');
        $expiredJob->setPosition('Web Developer Expired');
        $expiredJob->setLocation('Paris, France');
        $expiredJob->setDescription('Lorem ipsum dolor sit amet, consectetur adipisicing elit.');
        $expiredJob->setHowToApply('Send your resume to lorem.ipsum [at] dolor.sit');
        $expiredJob->setPublic(true);
        $expiredJob->setActivated(true);
        $expiredJob->setToken('job_expired');
        $expiredJob->setEmail('job@example.com');
        $expiredJob->setExpiresAt(new \DateTime('-10 days'));
